...
func CreateNamedPipe(name string, mode uint32, pipeMode uint32, maxInstances uint32, outBufferSize uint32, inBufferSize uint32, defaultTimeout uint32, sa *windows.SecurityAttributes) (handle windows.Handle, err error) {
	var _p0 *uint16
	_p0, err = syscall.UTF16PtrFromString(name)
	if err != nil {
		return
	}
	return _CreateNamedPipe(_p0, mode, pipeMode, maxInstances, outBufferSize, inBufferSize, defaultTimeout, sa)
}

func _CreateNamedPipe(name *uint16, mode uint32, pipeMode uint32, maxInstances uint32, outBufferSize uint32, inBufferSize uint32, defaultTimeout uint32, sa *windows.SecurityAttributes) (handle windows.Handle, err error) {
	r0, _, e1 := syscall.Syscall9(procCreateNamedPipeW.Addr(), 8, uintptr(unsafe.Pointer(name)), uintptr(mode), uintptr(pipeMode), uintptr(maxInstances), uintptr(outBufferSize), uintptr(inBufferSize), uintptr(defaultTimeout), uintptr(unsafe.Pointer(sa)), 0)
	handle = windows.Handle(r0)
	if handle == windows.InvalidHandle {
		err = errnoErr(e1)
	}
